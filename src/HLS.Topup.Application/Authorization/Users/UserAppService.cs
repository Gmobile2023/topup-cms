...
using HLS.Topup.Authorization.Users.Exporting;
...
namespace HLS.Topup.Authorization.Users
{
    ...
    public class UserAppService : TopupAppServiceBase, IUserAppService
    {
        ...
        public async Task<PagedResultDto<UserListDto>> GetUsers(GetUsersInput input)
        {
            ...
            var users = await query
                .OrderByDescending(x=>x.Id).ThenBy(x=>x.UserName)
                .PageBy(input)
                .ToListAsync();
        ...
        {
            if (input.User.AccountType == (byte) CommonConst.SystemAccountType.StaffApi)
            {
                if (input.User.ParentId == null)
                    throw new UserFriendlyException("Chưa chọn TK cấp trên");
                await _staffConfigurationManager.CreateUserStaff(new CreateStaffUserInput
                {
                    ...
                    ParentUserId = input.User.ParentId
                });
            }
            else
            {
                ...
                {
                    parentUser = await UserManager.GetUserByIdAsync(user.ParentId ?? 0);
                }
                else if (user.AccountType == CommonConst.SystemAccountType.MasterAgent)
                {
                    parentUser = await UserManager.GetUserByAccountTypeAsync(CommonConst.SystemAccountType.Company);
                }
                else if (user.AccountType == CommonConst.SystemAccountType.Company)
                {
                    var checkUser = await UserManager.GetUserByAccountTypeAsync(CommonConst.SystemAccountType.Company);
                    if (checkUser != null)
        ...
        private IQueryable<User> GetUsersFilteredQuery(IGetUsersInput input)
        {
            var query = UserManager.Users
                ...
                .WhereIf(input.IsSystem,
                    u => u.AccountType == CommonConst.SystemAccountType.System)
                .WhereIf(
                    ...
                    u =>
                        ...
                        u.AccountCode.Contains(input.Filter) ||
