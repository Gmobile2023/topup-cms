    ...
    public class TenantSettingsAppService : SettingsAppServiceBase, ITenantSettingsAppService
    {
        ...
        private async Task<SecuritySettingsEditDto> GetSecuritySettingsAsync()
        {
            ...
            return new SecuritySettingsEditDto
            {
                ...
                OtpSettings = await GetOtpSettingsAsync(),
        ...
        private Task<bool> IsOtpEnabledForApplicationAsync()
        {
            return SettingManager.GetSettingValueForApplicationAsync<bool>(AppSettings.UserManagement.OtpSetting.IsEnabled);
        }
        ...
        private async Task<OtpSettingsEditDto> GetOtpSettingsAsync()
        {
            var settings = new OtpSettingsEditDto
            {
                IsEnabled = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.OtpSetting.IsEnabled),
                IsOtpPass = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.OtpSetting.IsOtpPass),
                IsOdpVerificationEnabled = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.OtpSetting.IsOdpVerificationEnabled),
                IsOtpVerificationEnabled = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.OtpSetting.IsOtpVerificationEnabled),
                IsOtpIsEncrypted = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.OtpSetting.IsOtpIsEncrypted),
                MaxFailed = await SettingManager.GetSettingValueAsync<int>(AppSettings.UserManagement.OtpSetting.MaxFailed),
                OdpAvailable = await SettingManager.GetSettingValueAsync<int>(AppSettings.UserManagement.OtpSetting.OdpAvailable),
                OtpTimeOut = await SettingManager.GetSettingValueAsync<int>(AppSettings.UserManagement.OtpSetting.OtpTimeOut),
                OdpMaxSend = await SettingManager.GetSettingValueAsync<int>(AppSettings.UserManagement.OtpSetting.OdpMaxSend),
                IsUseOdpResetPass = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.OtpSetting.IsUseOdpResetPass),
                IsUseOdpRegister = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.OtpSetting.IsUseOdpRegister),
                IsUseOdpLogin = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.OtpSetting.IsUseOdpLogin),
                IsLevelPassVerificationEnabled = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.OtpSetting.IsLevelPassVerificationEnabled),
                DefaultVerifyTransId = await SettingManager.GetSettingValueAsync<byte>(AppSettings.UserManagement.OtpSetting.DefaultVerifyTransId),
                IsUseVerifyLogin = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.OtpSetting.IsUseVerifyLogin),
                IsUseVerifyRegister = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.OtpSetting.IsUseVerifyRegister),
                IsUseVerifyResetPass = await SettingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.OtpSetting.IsUseVerifyResetPass),
        ...
        {
            if (_multiTenancyConfig.IsEnabled && !TopupConsts.AllowTenantsToChangeEmailSettings)
            {
                return;
            }
        ...
        private async Task UpdateSecuritySettingsAsync(SecuritySettingsEditDto settings)
        {
            ...
            await UpdateOtpSettingsAsync(settings.OtpSettings);
        ...
        private async Task UpdateOtpSettingsAsync(OtpSettingsEditDto settings)
        {

            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.IsEnabled, settings.IsEnabled.ToString().ToLowerInvariant());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.IsOtpPass, settings.IsOtpPass.ToString().ToLowerInvariant());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.IsOdpVerificationEnabled, settings.IsOdpVerificationEnabled.ToString().ToLowerInvariant());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.IsOtpVerificationEnabled, settings.IsOtpVerificationEnabled.ToString().ToLowerInvariant());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.IsOtpIsEncrypted, settings.IsOtpIsEncrypted.ToString().ToLowerInvariant());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.MaxFailed, settings.MaxFailed.ToString());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.OdpAvailable, settings.OdpAvailable.ToString());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.OtpTimeOut, settings.OtpTimeOut.ToString());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.OdpMaxSend, settings.OdpMaxSend.ToString());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.IsUseOdpRegister, settings.IsUseOdpRegister.ToString());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.IsUseOdpResetPass, settings.IsUseOdpResetPass.ToString());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.IsUseOdpLogin, settings.IsUseOdpLogin.ToString());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.IsLevelPassVerificationEnabled, settings.IsLevelPassVerificationEnabled.ToString());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.DefaultVerifyTransId, settings.DefaultVerifyTransId.ToString());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.IsUseOdpLogin, settings.IsUseVerifyLogin.ToString());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.IsUseOdpLogin, settings.IsUseVerifyResetPass.ToString());
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.UserManagement.OtpSetting.IsUseOdpLogin, settings.IsUseVerifyRegister.ToString());
