...
using HLS.Topup.Migrations.Seed.Host;
...
namespace HLS.Topup.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : TopupAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<TopupDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{TopupConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{TopupConsts.ConnectionStringName}", constring);
        }
