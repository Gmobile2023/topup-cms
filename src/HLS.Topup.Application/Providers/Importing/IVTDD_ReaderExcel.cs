...
{
    public interface IVTDD_ReaderExcel : ITransientDependency
    {
        List<VTDDItem> GetCardsFromExcel(byte[] fileBytes);
    }
    public class VTDD_ReaderExcel : NpoiExcelImporterBase<VTDDItem>, IVTDD_ReaderExcel
    {
        private readonly ILocalizationSource _localizationSource;        
        private readonly ILogger<IVTDD_ReaderExcel> _logger;
        public VTDD_ReaderExcel(ILocalizationManager localizationManager, ILogger<IVTDD_ReaderExcel> logger)
        {
            _logger = logger;
            _localizationSource = localizationManager.GetSource(TopupConsts.LocalizationSourceName);
        }

        public List<VTDDItem> GetCardsFromExcel(byte[] fileBytes)
        {
            return ProcessExcelFile(fileBytes, ProcessExcelRow);
        }

        private VTDDItem ProcessExcelRow(ISheet worksheet, int row)
        {
