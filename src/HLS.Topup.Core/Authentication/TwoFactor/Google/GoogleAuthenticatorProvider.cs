...
namespace HLS.Topup.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : TopupServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("HLS.Topup", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
