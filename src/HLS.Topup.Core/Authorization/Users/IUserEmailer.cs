...
namespace HLS.Topup.Authorization.Users
{
    public interface IUserEmailer
    {
        ...
        Task SendEmailPinCode(User user, string emailSending, string transCode, List<CardResponseDto> listCard);

        Task SendEmailCreateAgentApi(User user, string email, string password, string cliendId, string clientKey);

        Task SendEmailPasswordFile(string emailSending, string fileName, string password);
    }
}