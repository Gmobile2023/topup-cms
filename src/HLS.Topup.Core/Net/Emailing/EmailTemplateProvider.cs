...
namespace HLS.Topup.Net.Emailing
{
    public class EmailTemplateProvider : IEmailTemplateProvider, ISingletonDependency
    {
        ...
        public string GetDefaultTemplate(int? tenantId)
        {
            ...
            {
                using (var stream = typeof(EmailTemplateProvider).GetAssembly().GetManifestResourceStream("HLS.Topup.Net.Emailing.EmailTemplates.default.html"))
                {
        ...
        public string GetTemplateByName(int? tenantId, string temName)
        {
            var tenancyKey = tenantId.HasValue ? tenantId.Value.ToString() : "host";
            return _defaultTemplates.GetOrAdd(tenancyKey, key =>
            {
                using (var stream = typeof(EmailTemplateProvider).GetAssembly().GetManifestResourceStream("HLS.Topup.Net.Emailing.EmailTemplates." + temName + ".html"))
                {
                    var bytes = stream.GetAllBytes();
                    var template = Encoding.UTF8.GetString(bytes, 3, bytes.Length - 3);
                    template = template.Replace("{THIS_YEAR}", DateTime.Now.Year.ToString());
                    template = template.Replace("{LINK_WEB_SITE}", _webUrlService.GetSiteRootAddress());
                    return template.Replace("{EMAIL_LOGO_URL}", GetTenantLogoUrl(tenantId));
                }
            });
        }
