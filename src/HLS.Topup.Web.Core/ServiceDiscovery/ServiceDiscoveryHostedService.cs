...
namespace HLS.Topup.Web.ServiceDiscovery
{
    public class ServiceDiscoveryHostedService : IHostedService
    {
        ...
        {
            _registrationId = $"{_config.ServiceName}-{_config.ServiceId}-{Guid.NewGuid():N}";
            ...
            if (_config.PingEnabled)
            {
                var httpCheck = new AgentServiceCheck()
                {
                    ...
                    HTTP = $"{_config.ServiceAddress.Scheme}://{_config.ServiceAddress.Host}:{_config.ServiceAddress.Port}{_config.PingEndpoint}",
