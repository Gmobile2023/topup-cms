...
var frontEndScripts = globby.sync([
    './wwwroot/themes/default/js/*.js',
    '!./wwwroot/themes/default/js/*.min.js'
]);

var frontEndStyles = globby.sync([
    './wwwroot/themes/default/css/*.css',
    './wwwroot/themes/default/css/*.less',
    '!./wwwroot/themes/default/css/*.min.css'
]);
...
function fillScriptBundles() {
    ...
    // FE scripts
    for (var i = 0; i < frontEndScripts.length; i++) {
        var viewScriptName = frontEndScripts[i].replace('./wwwroot/', '');
        scriptEntries[viewScriptName.replace('.js', '.min.js')] = [path.resolve(__dirname, frontEndScripts[i])];
    }
}
...
function fillStyleBundles() {
    ...
    // FE styles
    for (var j = 0; j < frontEndStyles.length; j++) {
        var viewStyleName = frontEndStyles[j].replace('./wwwroot/', '');
