(function ($) {
    app.modals.CreateOrEditDiscountModal = function () {
        ...
        let _DiscountuserLookupTableModal = new app.ModalManager({
            viewUrl: abp.appPath + 'App/Discounts/UserLookupTableModal',
            scriptUrl: abp.appPath + 'view-resources/Areas/App/Views/Discounts/_DiscountUserLookupTableModal.js',
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            return '<input class="discount form-control txtDiscount text-right" id="txtDiscount_' + row.productId + '" data-id=' + row.productId + ' type="text" data-amount=' + fixAmount + '"" value = ' + (data === null ? "" : (data === -1 ? "" : data)) + '   >';
                        }
                    },
                    {
                        targets: 5,
                        ...
                        class: "text-right",
                        render: function (data, type, row) {
                            return '<input class="fixAmount form-control amount-mask txtFixAmount text-right" id="txtFixAmount_' + row.productId + '"  data-id=' + row.productId + ' type="text"  value = ' + (data === null ? "" : (data === -1 ? "" : data)) + '   >';
                        }
                    },
                    {
                        targets: 6,
        ...
        $('#productMode').click(function() {
            ...
            _$discountsTable.DataTable().clear().draw();
        });
            ...
            if (files && files.length > 0) {
                ...
                Sv.AjaxPostFile2({
                    ...
                    $.each(data, function(i, e) {
                        if (parseFloat(e.discountValue) < -2 || parseFloat(e.discountValue) >= 100) {
                            format_error = true;
                            abp.message.warn("File Import có phần trăm chiết khấu không đúng định dạng!");
                            return false;
                        }

                        if (parseFloat(e.fixAmount) < -2) {
                            format_error = true;
                            abp.message.warn("File Import có số tiền chiết khấu không đúng định dạng!");
                            return false;
                        }
                    });
        ...
        this.save = function () {
            ...
            discount.listUserId = $('#UserId').val();
